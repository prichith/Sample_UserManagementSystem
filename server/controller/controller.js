...
exports.update = (req,res)=>{
    ...
    .then(data =>{
        if(!data){
            res.status(404).send({message:`Cannot connect user with ${id}. Maybe user not found`})
        }else{
            res.send(data)
        }
    })
...
exports.delete = (req,res)=>{
    ...
    .then(data =>{
        if(!data){
            res.status(404).send({message:`Cannot delete with the id ${id}. May be id is wrong.`})
        }else{
